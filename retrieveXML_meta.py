...
def get_entries_from_link(fr=0, maximum=1000, baselink=None):
    ...
    link = baselink+u'&max='+unicode(maximum)+u'&from='+unicode(fr)+ u'&date=2017-01-01'+u'&date=2017-05-01'
...
print 'begin\n'
c.execute(u''' DROP TABLE IF EXISTS heldout_uitspraken_meta''')
c.execute(u''' CREATE TABLE heldout_uitspraken_meta
            (id text PRIMARY KEY,
            title text,
            summary text,
            updated text
...
def insert_into_uitspraken_meta(entry, curs, table=u'uitspraken_meta'):
    ...
    query = u''' INSERT OR REPLACE INTO heldout_uitspraken_meta
        VALUES (?, ?, ?, ?)
    '''
