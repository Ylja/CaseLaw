...
print('\'Uitspraken\' with Metadata: '+ str(s[0]))

#How many 'uitspraken' do we have?
print('\'Uitspraken\': '+str(c.execute('''select count(*) from uitspraken''').fetchall()[0]))
...
wc = c.execute('''SELECT length(text) - length(replace(text, ' ', '')) + 1 FROM UITSPRAKEN''')
sum = 0
amount = 0
for text in wc.fetchall():
    ...
    amount += 1
avg = sum/amount
print('Average amount of words in \'uitspraken\': ' + str(avg) + '\n')
